...
async function getCloudinaryPhotos(gender) {
	...
	const url = `https://api.cloudinary.com/v1_1/matcha/resources/image/tags/${gender}`;
...
async function main() {
	...
	const [ okcupidJson, female, male ] = await Promise.all([okcupidPromise, femalePromise, malePromise]);
	const okcupid = JSON.parse(okcupidJson);
//	console.log(okcupid);
//	console.log(female);
//	console.log(male);
...
//fs.readFile('okcupid', 'ascii', function(err, contents) {
//
//	let okcupid = JSON.parse(contents);
//	
//
//	function genUser(num) {
//

...
const botLike = async (user) => {
	...
	const updateResponse = await Users.addLike({ receiver: user.id, sender: bot.id });

	logger.info(`Updating User { id: ${user.id} } score by 1`);
	const res = await Users.updateScore({ id: user.id, score: 10});
	if (res.error) { return res }

	return { data: bot };
};

module.exports = botLike;
